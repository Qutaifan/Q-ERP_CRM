...
if ($action == 'charge' && isModEnabled('stripe')) {	// Test on permission not required here (anonymous action protected by mitigation of /public/... urls)
			...
			if ($thirdparty_id > 0) {
				...
				if (!$error) {
					...
					if (empty($card)) {
						...
						$charge = $stripeClient->charges->create(array(
				...
				*/
				...
				$stripe = new Stripe($db);
				$stripeClient = $stripe->getStripeClient($servicestatus, $stripeacc);
				$customer = $stripeClient->customers->create(array(
					'email' => $email,
				...
				// Use StripeClient service-based pattern (stripe client already defined above)
				$charge = $stripeClient->charges->create(array(
					'customer' => $customer->id,
					'amount'   => price2num($amountstripe, 'MU'),
					'currency' => $currency,
				...
				), ['idempotency_key' => (string) $FULLTAG]);
				// Return $charge = array('id'=>'ch_XXXX', 'status'=>'succeeded|pending|failed', 'failure_code'=>, 'failure_message'=>...)
				if (empty($charge)) {
					$error++;
					dol_syslog('Failed to charge card', LOG_WARNING, 0, '_payment');
	...
	if (getDolGlobalInt('STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION')) {
		...
		try {
			// Use StripeClient service-based pattern
			$stripeClient = $stripe->getStripeClient($servicestatus, $stripeacc);
			$paymentintent = $stripeClient->paymentIntents->retrieve($paymentintent_id);
		} catch (Exception $e) {
			$error++;
