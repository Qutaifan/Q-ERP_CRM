		...
		if (getDolGlobalString('STRIPE_TEST_WEBHOOK_KEY') && getDolGlobalString('STRIPE_TEST_SECRET_KEY') && getDolGlobalString('STRIPE_TEST_WEBHOOK_ID')) {
			if (utf8_check(getDolGlobalString('STRIPE_TEST_SECRET_KEY'))) {
				try {
					...
					$stripe = new \Stripe\StripeClient(getDolGlobalString('STRIPE_TEST_SECRET_KEY'));
					$endpoint = $stripe->webhookEndpoints->retrieve(getDolGlobalString('STRIPE_TEST_WEBHOOK_ID'));
					
					// Update webhook endpoint
					$updateParams = ['enabled_events' => $stripearrayofwebhookevents, 'url' => $url];
					if (GETPOST('webhook', 'alpha') == getDolGlobalString('STRIPE_TEST_WEBHOOK_ID')) {
						$updateParams['disabled'] = !GETPOST('status', 'alpha');
					}
					$endpoint = $stripe->webhookEndpoints->update(getDolGlobalString('STRIPE_TEST_WEBHOOK_ID'), $updateParams);
	...
	if (getDolGlobalInt('MAIN_FEATURES_LEVEL') >= 2) {
		if (getDolGlobalString('STRIPE_LIVE_WEBHOOK_KEY') && getDolGlobalString('STRIPE_LIVE_SECRET_KEY') && getDolGlobalString('STRIPE_LIVE_WEBHOOK_ID')) {
			if (utf8_check(getDolGlobalString('STRIPE_TEST_SECRET_KEY'))) {
				try {
					// Use StripeClient service-based pattern
					$stripe = new \Stripe\StripeClient(getDolGlobalString('STRIPE_LIVE_SECRET_KEY'));
					$endpoint = $stripe->webhookEndpoints->retrieve(getDolGlobalString('STRIPE_LIVE_WEBHOOK_ID'));
					
					// Update webhook endpoint
					$updateParams = ['enabled_events' => $stripearrayofwebhookevents, 'url' => $url];
					if (GETPOST('webhook', 'alpha') == getDolGlobalString('STRIPE_LIVE_WEBHOOK_ID')) {
						$updateParams['disabled'] = empty(GETPOST('status', 'alpha'));
					}
					$endpoint = $stripe->webhookEndpoints->update(getDolGlobalString('STRIPE_LIVE_WEBHOOK_ID'), $updateParams);
...
if (getDolGlobalInt('MAIN_FEATURES_LEVEL') >= 2) {	// TODO Not used by current code
	...
	try {
		...
		if (!empty($site_account)) {
			// Use StripeClient service-based pattern
			$stripeClient = $stripe->getStripeClient($servicestatus);
			$stripeacc = $stripe->getStripeAccount($service);
			if ($stripeacc) {
				// Create new client with Connect account
				$stripeClient = $stripe->getStripeClient($servicestatus, $stripeacc);
			}
			$locations = $stripeClient->terminal->locations->all();
		}
	} catch (Exception $e) {
