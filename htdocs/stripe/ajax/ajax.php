...
if ($action == 'getConnexionToken') {
	try {
		...
		$connectionToken = $stripeClient->terminal->connectionTokens->create($array);
	...
	try {
		...
		$stripe = new Stripe($db);
		$stripeClient = $stripe->getStripeClient($servicestatus, $stripeacc);
		$intent = $stripeClient->paymentIntents->retrieve($json_obj->id);
		$intent = $stripeClient->paymentIntents->capture($json_obj->id);
