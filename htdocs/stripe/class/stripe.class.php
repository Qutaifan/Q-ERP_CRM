...
class Stripe extends CommonObject
{
	...
	/**
	 * Get a configured StripeClient instance
	 *
	 * @param	int<0,1>	$status				Status (0=test, 1=live)
	 * @param	string		$stripeconnectaccount	Optional Stripe Connect account 'acc_....' to use
	 * @return 	\Stripe\StripeClient			Configured StripeClient instance
	 */
	public function getStripeClient($status = 0, $stripeconnectaccount = '')
	{
		global $stripearrayofkeysbyenv;

		$config = array(
			'api_key' => $stripearrayofkeysbyenv[$status]['secret_key'],
			'stripe_version' => getDolGlobalString('STRIPE_FORCE_VERSION', '2022-11-15'),
		);

		// Add Stripe Connect account if specified
		if (!empty($stripeconnectaccount)) {
			$config['stripe_account'] = $stripeconnectaccount;
		}
	...
	public function customerStripe($object, $key = '', $status = 0, $createifnotlinkedtostripe = 0)
	{
		...
		global $stripearrayofkeysbyenv;
		$stripe = $this->getStripeClient($status, $key);
		...
		if ($resql) {
			...
			if ($num) {
				...
				try {
					// Use StripeClient service-based pattern
					$customer = $stripe->customers->retrieve($tiers, ['expand' => ['sources']]);
				} catch (Exception $e) {
				...
				*/
				...
				try {
					// Use StripeClient service-based pattern (client already configured with Connect account if needed)
					$customer = $stripe->customers->create($dataforcustomer);
	...
	public function getPaymentMethodStripe($paymentmethod, $key = '', $status = 0)
	{
		...
		try {
			// Get StripeClient instance (with Connect account if specified)
			$stripe = $this->getStripeClient($status, $key);
			// Use StripeClient service-based pattern
			$stripepaymentmethod = $stripe->paymentMethods->retrieve((string) $paymentmethod->id);
		} catch (Exception $e) {
			$this->error = $e->getMessage();
		}
	...
	public function getSelectedReader($reader, $key = '', $status = 0)
	{
		...
		try {
			...
			$stripe = $this->getStripeClient($status, $key);
			// Use StripeClient service-based pattern
			$selectedreader = $stripe->terminal->readers->retrieve((string) $reader);
		} catch (Exception $e) {
			$this->error = $e->getMessage();
		}
	...
	public function getPaymentIntent($amount, $currency_code, $tag, $description = '', $object = null, $customer = null, $key = null, $servicestatus = 0, $usethirdpartyemailforreceiptemail = 0, $mode = 'automatic', $confirmnow = false, $payment_method = null, $off_session = 0, $noidempotency_key = 1, $did = 0)
	{
		...
		if (is_object($object) && getDolGlobalInt('STRIPE_REUSE_EXISTING_INTENT_IF_FOUND') && !getDolGlobalInt('STRIPE_CARD_PRESENT')) {
			...
			if ($resql) {
				...
				if ($num) {
					...
					try {
						// Get StripeClient instance (with Connect account if specified)
						$stripe = $this->getStripeClient($servicestatus, $key);
						// Use StripeClient service-based pattern
						$paymentintent = $stripe->paymentIntents->retrieve($intent);
					} catch (Exception $e) {
						$error++;
						$this->error = $e->getMessage();
					}
		...
		if (empty($paymentintent)) {
			...
			try {
				...
				// Handle idempotency key if required
				$requestOptions = [];
				if (empty($noidempotency_key)) {
					$requestOptions['idempotency_key'] = $descriptioninpaymentintent;
				}
				...
				if (!empty($requestOptions)) {
					$paymentintent = $stripe->paymentIntents->create($dataforintent, $requestOptions);
				} else {
					$paymentintent = $stripe->paymentIntents->create($dataforintent);
				}
	...
	public function getSetupIntent($description, $object, $customer, $key, $servicestatus, $usethirdpartyemailforreceiptemail = 0, $confirmnow = false)
	{
		...
		if (empty($setupintent)) {  // @phan-suppress-current-line PhanPluginConstantVariableNull
			...
			try {
				...
				// Use StripeClient service-based pattern
				$setupintent = $stripe->setupIntents->create($dataforintent);
		...
		if ($resql) {
			$num = $this->db->num_rows($resql);
				...
				if ($cardref) {
					try {
						// Get StripeClient instance (with Connect account if specified)
						$stripe = $this->getStripeClient($status, $stripeacc);

						if (!preg_match('/^pm_/', $cardref) && !empty($cu->sources)) {
							// Legacy card sources - still use customer sources object
							$card = $cu->sources->retrieve($cardref);
						} else {
							// Use StripeClient service-based pattern for PaymentMethods
							$card = $stripe->paymentMethods->retrieve($cardref);
						}
					} catch (Exception $e) {
						$this->error = $e->getMessage();
			...
			if ($num) {
				...
				if ($cardref) {
					try {
						...
						if (!preg_match('/^pm_/', $cardref) && !empty($cu->sources)) {
							// Legacy sources - still use customer sources object
							$sepa = $cu->sources->retrieve($cardref);
						} else {
							// Use StripeClient service-based pattern for PaymentMethods
							$sepa = $stripe->paymentMethods->retrieve($cardref);
						}
					} catch (Exception $e) {
						$this->error = $e->getMessage();
						dol_syslog($this->error, LOG_WARNING);
